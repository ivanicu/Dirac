...
const PostprocessingSchema = {
    ...
    dToneMappingEnable: DefineSpec('boolean'),
    dVignetteEnable: DefineSpec('boolean'),
    uVignetteStrength: UniformSpec('f'),
...
function getPostprocessingRenderable(ctx: WebGLContext, colorTexture: Texture, transparentColorTexture: Texture, depthTextureOpaque: Texture, depthTextureTransparent: Texture, shadowsTexture: Texture, outlinesTexture: Texture, ssaoDepthTexture: Texture, ssaoDepthTransparentTexture: Texture, bloomTexture: Texture, transparentOutline: boolean): PostprocessingRenderable {
    const values: Values<typeof PostprocessingSchema> = {
        ...
        dToneMappingEnable: ValueCell.create(false),
        dVignetteEnable: ValueCell.create(false),
        uVignetteStrength: ValueCell.create(0),
        ...
        uShadowStrength: ValueCell.create(1),
...
export const PostprocessingParams = {
    ...
    toneMapping: PD.MappedStatic('off', {
        on: PD.Group({}),
        off: PD.Group({})
    }, { cycle: true, description: 'Filmic highlight rolloff and midtone response', hideIf: p => p.enabled === false }),
    vignette: PD.MappedStatic('off', {
        on: PD.Group({ strength: PD.Numeric(0.42, { min: 0, max: 1, step: 0.01 }) }),
        off: PD.Group({})
    }, { cycle: true, description: 'Frame-edge darkening applied after geometry compositing', hideIf: p => p.enabled === false }),
...
export class PostprocessingPass {
    ...
    updateState(camera: ICamera, scene: Scene, transparentBackground: boolean, backgroundColor: Color, props: PostprocessingProps, light: Light, ambientColor: Vec3) {
        ...
        const toneMappingEnabled = props.toneMapping.name === 'on';
        const vignetteEnabled = props.vignette.name === 'on';
        ...
        if (shadowsEnabled) {
            const shadowProps = props.shadow.params as ShadowProps;
            this.shadow.update(camera, light, ambientColor, shadowProps);
            ValueCell.updateIfChanged(this.renderable.values.uShadowStrength, shadowProps.strength);
        }
        ...
        if (this.renderable.values.dToneMappingEnable.ref.value !== toneMappingEnabled) {
            needsUpdateMain = true;
            ValueCell.update(this.renderable.values.dToneMappingEnable, toneMappingEnabled);
        }
        if (vignetteEnabled) {
            ValueCell.updateIfChanged(this.renderable.values.uVignetteStrength, (props.vignette.params as { strength: number }).strength);
        }
        if (this.renderable.values.dVignetteEnable.ref.value !== vignetteEnabled) {
            needsUpdateMain = true;
            ValueCell.update(this.renderable.values.dVignetteEnable, vignetteEnabled);
        }
