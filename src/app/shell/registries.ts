...
const PROGRAM_REFERENCE_COMMANDS = [
    'program.target_disease.link', 'identity.substance_registration.record', 'material.batch.register',
...
export const WORKSPACES: readonly WorkspaceDefinition[] = [
    ...
    { id: 'campaigns', label: 'Campaigns', icon: '⊞', defaultView: 'campaigns.compounds', availability: 'implemented', shellReady: true },
    { id: 'synthesis', label: 'Synthesis', icon: '⇝', defaultView: 'synthesis.make', availability: 'implemented', shellReady: true },
    { id: 'experiments', label: 'Experiments', icon: '◫', defaultView: 'experiments.design', availability: 'implemented', shellReady: true },
    { id: 'knowledge', label: 'Knowledge', icon: '▦', defaultView: 'knowledge.evidence', availability: 'implemented', shellReady: true },
...
export const VIEWS: readonly ViewDefinition[] = [
    ...
    view('design.builder', 'design', 'Builder', '/p/:programId/design/builder', true,
        ['chem.builder', 'chem.property-summary'], ['molecule'], ['conformer.generate', 'molecule.properties', 'compound.register']),
    ...
    view('campaigns.compounds', 'campaigns', 'Compounds', '/p/:programId/campaigns/compounds', true,
        ['program.campaign-records'], ['program', 'compound', 'series'],
        ['compound.register', 'identity.substance_registration.record', 'program.link']),
    ...
    view('synthesis.make', 'synthesis', 'Make Queue', '/p/:programId/synthesis/make', true,
        ['program.material-records'], ['program', 'compound', 'batch', 'sample'],
        ['material.batch.register', 'sample.create', 'sample.transfer', 'program.lineage.record']),
    view('experiments.design', 'experiments', 'Design', '/p/:programId/experiments/design', true,
        ['program.experiment-records'], ['program', 'sample', 'protocol_version', 'experiment'],
        ['protocol.version.record', 'experiment.record', 'dataset.version.commit']),
    ...
    view('knowledge.evidence', 'knowledge', 'Evidence', '/knowledge/evidence', true,
        ['program.knowledge-records'], ['program', 'evidence', 'dataset_version', 'measurement'],
        ['evidence.release.import', 'evidence.external.record', 'program.evidence.attach']),
...
export const MODULES: readonly ModuleDefinition[] = [
    ...
    { id: 'program.campaign-records', version: 1, supportedViews: ['campaigns.compounds'], requiresContext: ['program'], consumesObjects: ['program', 'compound', 'series', 'substance_registration'], providesCommands: ['compound.register', 'identity.substance_registration.record', 'program.link'], surfaces: ['ledger'], placement: 'main', priority: 100 },
    { id: 'program.material-records', version: 1, supportedViews: ['synthesis.make'], requiresContext: ['program'], consumesObjects: ['program', 'compound', 'compound_form', 'batch', 'sample'], providesCommands: ['material.batch.register', 'sample.create', 'sample.transfer', 'program.lineage.record'], surfaces: ['ledger'], placement: 'main', priority: 100 },
    { id: 'program.experiment-records', version: 1, supportedViews: ['experiments.design'], requiresContext: ['program'], consumesObjects: ['program', 'sample', 'protocol_version', 'experiment', 'dataset_version'], providesCommands: ['protocol.version.record', 'experiment.record', 'dataset.version.commit'], surfaces: ['ledger'], placement: 'main', priority: 100 },
    { id: 'program.knowledge-records', version: 1, supportedViews: ['knowledge.evidence'], requiresContext: ['program'], consumesObjects: ['program', 'evidence', 'dataset_version', 'measurement', 'external_evidence_record'], providesCommands: ['evidence.release.import', 'evidence.external.record', 'program.evidence.attach'], surfaces: ['ledger'], placement: 'main', priority: 100 },
    ...
    { id: 'chem.builder', version: 2, supportedViews: ['design.builder'], requiresContext: [], consumesObjects: ['molecule'], providesCommands: ['conformer.generate', 'compound.register'], surfaces: ['focus', 'ligand'], placement: 'main', priority: 100 },
