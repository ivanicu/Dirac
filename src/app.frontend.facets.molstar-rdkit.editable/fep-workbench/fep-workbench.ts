...
function bindAuthoritativeCampaignContext(value:Network,networkJobId:string,metadataHint:CampaignContext|null):CampaignContext|null {
    ...
    if (context) { copyStorage.set('dirac.rbfe.active_network_job_id',networkJobId); copyStorage.set('dirac.rbfe.active_campaign_context',JSON.stringify(context)); } else { copyStorage.remove('dirac.rbfe.active_campaign_context'); if (networkJobId!==DefaultNetworkJobId)copyStorage.remove('dirac.rbfe.active_network_job_id'); }
    return context;
}
...
async function atomInfo(smiles: string): Promise<AtomInfo> {
    ...
    try {
        ...
        try {
            ...
            const aromaticAtoms=new Set<number>(); let aromaticQuery:ReturnType<typeof RDKit.get_qmol>|null=null;
            try { aromaticQuery=RDKit.get_qmol('[a]'); const matches=aromaticQuery?JSON.parse(mol.get_substruct_matches(aromaticQuery)) as Array<{atoms?:number[]}>:[]; if (Array.isArray(matches))matches.forEach(match=>match.atoms?.forEach(index=>aromaticAtoms.add(index))); } finally { aromaticQuery?.delete(); }
            ...
            potentialEzBonds=bonds.filter(bond=>isPotentialEzBond(bond,aromaticAtoms)&&!cipBonds.some(([left,right])=>(left===bond.left&&right===bond.right)||(left===bond.right&&right===bond.left))).filter(bond=>{ const left=substituentKeys(bond.left,bond.right),right=substituentKeys(bond.right,bond.left); return left.length===2&&right.length===2&&left[0]!==left[1]&&right[0]!==right[1]; }).map(bond=>[bond.left,bond.right]);
...
async function replanNetwork(): Promise<void> {
    ...
    } catch (error) { if (receipt&&plannerReceiptOwned(receipt.owner_token)) { const owned=plannerReceiptOwned(receipt.owner_token)!; writePlannerReceipt({ ...owned,status: owned.job_id?'detached':'submitting' }); } if (current())text('status',error instanceof Error?error.message:String(error)); } finally { if (current()) { button.disabled=false; button.textContent=priorContext?'REPLAN NETWORK':'REPLAN BENCHMARK'; } }
}
...
function syncExecutionContract(): void {
    ...
    document.querySelectorAll<HTMLInputElement|HTMLSelectElement|HTMLTextAreaElement>('#campaign-builder [data-invalidate],#campaign-pdb,#campaign-ligands').forEach(element=>element.disabled=locked); document.querySelectorAll<HTMLButtonElement>('#campaign-builder [data-choice-group] button').forEach(element=>element.disabled=locked||element.dataset.availability==='unavailable'); const clear=document.getElementById('clear-campaign') as HTMLButtonElement|null; if (clear)clear.disabled=physicalRunActive()||!!readPlannerReceipt();
...
function invalidateLigandValidation():void {
    ...
    const parent=document.getElementById('parent-compound-select') as HTMLSelectElement|null; if (parent) { parent.disabled=true; parent.innerHTML='<option value="">REVALIDATE THE SERIES FIRST</option>'; }
}
...
function updateBuilderReadiness(validLigands=validatedLigandSignature===currentLigandSignature()?validatedLigandCount:0):void {
    ...
    const parent=(document.getElementById('parent-compound-select') as HTMLSelectElement|null)?.value||'',parentReady=allLigandsReady&&!!parent&&textLigandRows().some(row=>row.id===parent); const reference=selectedReferenceLigand(); const structureReady=receptorInputReady&&!!receptorPdbText&&!!reference; const ready=structureReady&&parentReady&&decisionReady;
    ...
    if (ligandGate) { const icon=ligandGate.querySelector('i'),detail=ligandGate.querySelector('small'); if (icon)icon.textContent=parentReady?'✓':validatedLigandErrorCount?'!':'○'; if (detail)detail.textContent=parentReady?`${validLigands} identities validated · ${parent} explicitly selected as reference`:allLigandsReady?'Choose which validated compound is the reference':validatedLigandErrorCount?`${validatedLigandErrorCount} explicit row/policy error${validatedLigandErrorCount===1?'':'s'} block review`:validLigands?'Validation is stale or fewer than two rows are ready':'No new molecules yet'; }
    if (portfolioGate) { const icon=portfolioGate.querySelector('i'),detail=portfolioGate.querySelector('small'); if (icon)icon.textContent=decisionReady?'✓':'○'; if (detail)detail.textContent=decisionReady?`Decision recorded · ${estimate.edges} edges · ${estimate.jobs} jobs · ~${estimate.gpuHours} GPU hours`:[...decision.missing,...decision.invalid].slice(0,2).join(' · '); }
    renderDecisionValidation(document,decision);
}
...
function openMoleculeSketcher():void {
    ...
    try { moleculeSketcher??=new MoleculeSketcher(sketcherHost,addSketchedMolecule); moleculeSketcher.open(); } catch (error) { showBuilderNotice(`2D editor unavailable · paste SMILES remains available · ${error instanceof Error?error.message:String(error)}`); }
}
...
function selectChoice(button:HTMLButtonElement):void {
    ...
    if (button.disabled||button.dataset.availability==='unavailable') { showBuilderNotice(`${button.textContent?.trim()||'This option'} is not available in the connected workflow.`); return; }
    ...
    group.querySelectorAll('button').forEach(item=>{ item.classList.toggle('active',item===button); item.setAttribute('aria-pressed',String(item===button)); });
...
function textLigandRows():LigandInputRow[] {
    ...
    raw.split(/\r?\n/).forEach((source,sourceIndex)=>{
        ...
        if (tokens.length===1) { id=`CMPD-${String(sourceIndex+1).padStart(3,'0')}`; smiles=tokens[0]; } else if (tokens.length===2) { [id,smiles]=tokens; if (!/^[A-Za-z0-9][A-Za-z0-9_.:+-]{0,127}$/.test(id))parseError='ID must start with a letter or digit and use only letters, digits, _ . : + -'; } else { id=tokens[0]||`ROW-${sourceIndex+1}`; smiles=tokens.slice(1).join(' '); parseError=`Expected exactly SMILES or ID + SMILES; found ${tokens.length} whitespace/comma tokens`; }
...
async function validateBuilderLigands():Promise<{rows:Array<{id:string;smiles:string}>;valid:number;charged:number}> {
    ...
    for (const row of inputRows) {
        let canonical='NOT GENERATED',charge='UNVERIFIED',stereo='UNVERIFIED',outcome='ERROR · INPUT ROW NOT PARSED',rowError=row.parseError||'',depiction='';
        if (!rowError) {
            try {
                const info=await atomInfo(row.smiles); if (validationId!==ligandValidationGeneration||epoch!==draftEpoch||capturedSignature!==currentLigandSignature()) return { rows: inputRows.map(({ id,smiles })=>({ id,smiles })),valid: 0,charged: 0 }; const formalCharge=info.charges.reduce((sum,value)=>sum+value,0),stereoState=stereoIdentity(info); canonical=info.canonicalSmiles; charge=`${formalCharge>0?'+':''}${formalCharge} · ${formalCharge===0?'NEUTRAL':'NONZERO FORMAL CHARGE'}`; stereo=stereoState.label; depiction=await moleculeSvg(canonical,150,96);
    ...
    renderLigandAuditRows(document,rendered.join(''));
...
let draftCampaignId:string=workbenchUuid();
...
function scientificInputsFromUi(rows:Array<{id:string;smiles:string}>):Record<string,unknown>|null {
    ...
    const parentId=(document.getElementById('parent-compound-select') as HTMLSelectElement|null)?.value||'',parent=rows.find(row=>row.id===parentId); if (!parent) return null;
...
document.getElementById('save-draft')?.addEventListener('click',async event=>{
    ...
    try {
        ...
        if (receipt.durability==='server') { await loadSystemCatalog(); text('builder-next-label',`SERVER-SAVED · CAMPAIGN V${receipt.version}`); showBuilderNotice(`SERVER-DURABLE draft saved · ${receipt.receipt} · no compute job created.`); } else { text('builder-next-label','OFFLINE CACHE ONLY · SERVER SAVE REQUIRED'); showBuilderNotice(`${receipt.warning} · ${receipt.receipt} · local cache is recoverable but is not a durable campaign receipt.`); }
    } catch (error) { const message=error instanceof Error?error.message:String(error); text('builder-next-label',message.startsWith('VERSION CONFLICT')?'VERSION CONFLICT · RESUME + REAPPLY':'DRAFT SAVE REFUSED'); showBuilderNotice(`Draft save failed · ${message}`); } finally { button.disabled=false; button.textContent='SAVE DRAFT'; }
});
document.getElementById('resume-draft')?.addEventListener('click',async event=>{
    ...
    try {
        ...
        if (loaded.source==='server') { await loadSystemCatalog(); if (!operations.current(restored,{ edits: true })) return; text('builder-next-label',`SERVER-RESTORED · CAMPAIGN V${loaded.version}`); showBuilderNotice(`SERVER-DURABLE campaign restored · ${loaded.draft.campaign_id} · v${loaded.version} · generated evidence was not silently rebound.`); } else { text('builder-next-label','OFFLINE CACHE RESTORED · NOT SERVER-DURABLE'); showBuilderNotice(`${loaded.warning||'NOT SERVER-DURABLE'} · offline inputs restored; save to the server before preparation or import.`); }
    } catch (error) { showBuilderNotice(`Draft resume failed · ${error instanceof Error?error.message:String(error)}`); } finally { button.disabled=false; button.textContent='RESUME DRAFT'; }
});
...
if (!current()) return; builderStage='reviewed'; reflectBuilderStage(); text('builder-next-label',`PREP JOB ${receipt.job_id} · INPUTS PRESERVED · FAILED OR STILL QUERYABLE`); text('pose-review-state','NOT GENERATED'); const preserved=!!readPreparationReceipt(); showBuilderNotice(`Preparation job ${receipt.job_id}: ${error instanceof Error?error.message:String(error)}. Scientific inputs remain unchanged and the full receipt is ${preserved?'preserved for resume':'recorded as terminal'}.`); button.textContent=preserved?'RESUME PREPARATION JOB':'RETRY RECEPTOR + POSES →';
...
async function createCampaignNetwork():Promise<void> {
    ...
    } catch (error) { const owned=plannerReceiptOwned(scope.id); if (owned)writePlannerReceipt({ ...owned,status: owned.job_id?'detached':'submitting' }); if (currentPlannerCancelState()!=='confirmed'&&currentPlannerCancelState()!=='stop-wait'&&current()) { showBuilderNotice(`Campaign creation failed: ${error instanceof Error?error.message:String(error)}`); text('builder-next-label',currentPlannerCancelState()==='failed'?'CANCEL FAILED · PLANNER RECEIPT STILL ACTIVE':'PLANNER FAILED · INPUTS + RECEIPT PRESERVED'); } } finally { if (plannerOwnerToken===scope.id) { window.clearInterval(plannerProgressTimer); plannerWaitController=null; plannerDetachWait=null; plannerInFlight=false; plannerOwnerToken=''; plannerJobId=''; button.disabled=false; button.textContent=builderStage==='accepted'?'PLAN / RETRY OPENFE NETWORK →':'REVIEW POSES IN 3D →'; } }
}
...
function getPoseReviewer():PoseReviewer|null {
    ...
    try { return poseReviewer??=new PoseReviewer(poseReviewHost,apiBase,acceptReviewedPoses); } catch (error) { showBuilderNotice(`Pose viewer initialization failed · tabular evidence fallback required · ${error instanceof Error?error.message:String(error)}`); return null; }
}
