...
void main(void) {
    ...
    float transparentDepth = 1.0;
        ...
        if (!isOpaqueBackground) {
            ...
            float shadowFactor = mix(1.0, shadow.a, uShadowStrength);
            if (!uTransparentBackground) {
                color.rgb = mix(mix(vec3(0), uFogColor, fogFactor), color.rgb, shadowFactor);
            } else {
                color.rgb = mix(vec3(0) * (1.0 - fogFactor), color.rgb, shadowFactor);
            }
        }
    #endif
    ...
    #ifdef dToneMappingEnable
        // ACES-inspired fitted filmic curve: cheap, monotonic and free of LUT texture fetches.
        vec3 x = max(color.rgb, vec3(0.0));
        color.rgb = clamp((x * (2.51 * x + 0.03)) / (x * (2.43 * x + 0.59) + 0.14), 0.0, 1.0);
    #endif

    #ifdef dVignetteEnable
        // Smooth, resolution-independent edge falloff. Kept after bloom and tone mapping so
        // the frame is composed consistently without modifying molecular colors or loci.
        vec2 vignetteCoords = coords * 2.0 - 1.0;
        vignetteCoords.x *= uTexSize.x / uTexSize.y;
        float vignette = smoothstep(0.28, 1.32, dot(vignetteCoords, vignetteCoords));
