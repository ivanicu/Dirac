...
class _Cursor:
    def __init__(self, owner: tuple[str, str], data: bytes,
                 authorization_schema=(True, True, True, True)) -> None:
        self.owner = owner
        self.data = data
    ...
    def execute(self, sql, params=()):
        ...
        assert "app.research_loop_artifact" in statement
        supplied = tuple(params[-10:])
        expected = self.owner * 5
...
def _store(owner=("human", "chemist-a"), data=b"private pose",
           authorization_schema=(True, True, True, True)):
...
def test_authorization_fails_closed_before_campaign_owner_schema_exists():
    store, cursor, _ = _store(authorization_schema=(True, False, False, True))
...
def test_owner_can_read_and_every_authorization_route_is_present():
    ...
    assert "app.research_loop_artifact" in access_sql
...
def test_authorization_fails_closed_before_research_loop_owner_schema_exists():
    store, cursor, _ = _store(authorization_schema=(True, True, True, False))
    with pytest.raises(failures.DiracFailure) as caught:
        store.head_authorized(
            str(UUID(int=7)), {"kind": "human", "id": "chemist-a"})
    assert caught.value.code == "DB_UNAVAILABLE"
    assert caught.value.details["required_migrations"] == [
        "049_research_loop.sql"]
    assert not any("FROM app.artifact a" in sql for sql in cursor.statements)
