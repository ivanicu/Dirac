...
const Apps = [
    ...
    { kind: 'app', name: 'dirac', filename: 'dirac.js', entryRoot: './src/app.frontend.facets.molstar-rdkit.editable', staticDirs: ['assets/rdkit', 'fonts'], staticFiles: ['fonts.css','fascia-tokens.css','theme-fascia.css','theme-fascia.js','workspace-shell.css','voice-composer.css','addons.css','palette.js', ['../../node_modules/frappe-gantt/dist/frappe-gantt.css', 'frappe-gantt.css']], auxiliaryEntries: [{ entry: 'voice-composer-demo.ts', outfile: 'voice-composer-demo/voice-composer-demo.js', staticFiles: [['voice-composer-demo.html', 'voice-composer-demo/index.html'], ['voice-composer.css', 'voice-composer-demo/voice-composer.css']] }, { entry: 'fep-workbench/fep-workbench.ts', outfile: 'time-tunnel/fep-workbench.js', staticFiles: [['fep-workbench/fep-workbench.html', 'time-tunnel/fep-workbench.html'], ['fep-workbench/fep-workbench.css', 'time-tunnel/fep-workbench.css'], ['fep-workbench/research-loop-panel.css', 'time-tunnel/research-loop-panel.css']] }] },
...
function fileLoaderPlugin(options) {
    ...
    return {
        ...
        setup(build) {
            ...
            build.onLoad({ filter: /\.(glb|cif|pdb)$/ }, async (args) => {
                try {
                    const name = path.basename(args.path);
                    mkDir(path.resolve(options.out, 'assets'));
                    await fs.promises.copyFile(args.path, path.resolve(options.out, 'assets', name));
                    return {
                        contents: `./assets/${name}`,
                        loader: 'text',
                    };
                } catch (error) {
                    handleFileError(error, 'copy', args.path);
